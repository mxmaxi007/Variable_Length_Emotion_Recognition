...
def dynamic_rnn(input_data, hidden_size):
    ...
    outputs, state = tf.nn.bidirectional_dynamic_rnn(rnn_cell, rnn_cell, input_data, dtype=tf.float32);

    fw_outputs = outputs[0][:, -1];
...
def model_design(class_num, feature_size):
    ...
    with tf.name_scope("conv1"):
        W_conv1 = weight_variable([1, 1, 12, 6]);
    ...
    with tf.name_scope("conv2"):
        W_conv2 = weight_variable([1, 1, 8, 10]);
...
def expand_list(x_list, y_list):
    X = [];
    Y = [];

    for i in range(len(x_list)):
        for spectrogram in x_list[i]:
        ...
        for i in range(epoch_num):
            ...
            # saver.save(sess, os.path.join(ckpt_dir, str(i) + ".ckpt"));
                ...
                if val_acc >= best_val_acc:
                    ...
                    builder.add_meta_graph_and_variables(sess, tags=["CNN_RNN_Const"]);
                    builder.save();
